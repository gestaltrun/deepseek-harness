...
try {
  ...
  const instanceList = await waitFor(client.im.simulationInstances, state => state.phase === 'ready', 'Simulation instance list')
  assert.deepEqual(Array.from(instanceList.value), [])
  const simulationSession = client.im.watchSimulationSession('unbound-session')
  const unbound = await waitFor(simulationSession, state => state.phase === 'ready', 'Unbound simulation Session')
  assert.equal(unbound.value.sessionId, 'unbound-session')
  assert.equal(unbound.value.scope, undefined)
  assert.equal(unbound.value.instance, undefined)
  assert.deepEqual(Array.from((await client.im.listSimulationInstances()).value), [])
  assert.equal((await client.im.scopeForSession('unbound-session')).value, undefined)
  assert.equal((await client.im.getSimulationInstance('missing-instance')).value, undefined)
  await simulationSession.dispose()
  ...
  assert.equal(TYPERT.invocations.length, 33)
