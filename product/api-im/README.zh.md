...
可观察对象保持稳定身份，批量发送结构变更通知，并在销毁时移除观察者。连接取消会关闭 Host 订阅。候选发现为每个平台保留独立 Client 对象；已取消或已被替代的读取不能覆盖更新的选项。UI 草稿与选择由消费本包的 UI 插件持有。
...
模拟 reader 投影 Host 的完整实例列表，以及一个已选 Session 的权威角色、对端 Session、冻结目标和投递 scope。实例列表用于装饰 Session 行；Session reader 固定一个不可变 Session 身份，直到导航将其销毁。创建、参与者注入、受控人工注入和停止都委托给公开运行时方法。停止分为两步：UI 先等待持久状态变为 `stopping`，再等待 Host 返回终态结果。
