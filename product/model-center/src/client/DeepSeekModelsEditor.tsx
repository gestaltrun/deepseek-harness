...
import { modelCapabilityError } from './ModelCapabilitiesFields.tsx'
...
export interface DeepSeekModelsValidationFailure {
  ...
  | 'modelMaxTokensInvalid' | 'modelDefaultInvalid' | 'modelReasoningInvalid'
}
...
export function validateDeepSeekModels(value: unknown): DeepSeekModelsValidationFailure | undefined {
  ...
  for (const [index, model] of models.entries()) {
    ...
    const capabilityError = modelCapabilityError(model)
    if (capabilityError !== undefined) return { index, key: capabilityError }
    const id = model['id']
