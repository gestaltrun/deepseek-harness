...
const id = '@gestaltrun/dsh-model-center'
  ...
  { entry: { client: 'src/client/index.ts' }, format: 'cjs', platform: 'browser', target: 'es2022',
    ...
    plugins: [{
      ...
      resolveId(source, importer) {
        ...
        return '\0model-center-css:' + relative(import.meta.dirname, file).split(sep).join('/') + '.mjs'
      },
      async load(moduleId) {
        if (!moduleId.startsWith('\0model-center-css:')) return null
        const relativePath = moduleId.slice('\0model-center-css:'.length, -4)
