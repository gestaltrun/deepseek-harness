...
账号分别展示授权与监听事实。接入使用供应方发现的身份，只通过 Host 的只写操作发送凭据字段。接入失败保持可见，不会将账号标记为已连接。暂停、断开、重连与刷新授权会使用已显示的账号版本发送生命周期命令。断开确认使用打开时捕获的账号名称、路由数量与版本。操作处于待完成或结果未知状态时，其他账号命令保持禁用；API 提供账号操作查询前，UI 无法核对未知结果。
