...
定向测试覆盖接入点准入、凭据清除、身份确认、setup 取消竞态、确认重放、生命周期回执查询、多目标部分结果和适配后的 Workspace browser。这些检查不等于完整安装版 GUI 验收。模拟目标选择和清除使用运行时的条件路由引用与回执查询；未知响应保留操作身份供后续核对。创建模拟、权威 Session 到 scope 的导航及停止控件仍待对应 Host API。目标卡片和 IM tab 明确显示这些执行与查看限制。
