...
定向测试覆盖接入点准入、凭据清除、身份确认、setup 取消竞态、确认重放、生命周期回执查询、多目标部分结果和适配后的 Workspace browser。这些检查不等于完整安装版 GUI 验收。模拟目标选择和清除使用运行时的条件路由引用与回执查询；未知响应保留操作身份供后续核对。目标选择器分别展示真实连接意图、授权过期、规则启用及身份资料可用性；仅监听闲置不禁用模拟。Session 列表跟随持久实例并标记双方，包括已停止状态。IM tab 从已选模拟用户 Session 创建实例，使用 Host 提供的对端与投递 scope，注入已准入参与者或受控人工消息，并在确认停止后等待终态结果。公开 Tool-view slot 为四种模拟调用展示紫色通道标记、持久输入和结果及隔离说明。冻结目标事实与后续配置漂移持续可见，已停止及失败实例保持只读。
