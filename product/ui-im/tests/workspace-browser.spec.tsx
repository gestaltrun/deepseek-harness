...
import type { WorkspaceBrowserProps } from '../src/client/workspace/contract/slots.ts'
import { createWorkspaceViewStore, FLAT_SESSION_ORDER_KEY } from '../src/client/workspace/stores.ts'
import { UNGROUPED_KEY } from '../src/client/workspace/tree.ts'
import { WorkspaceBrowser } from '../src/client/workspace/rows/WorkspaceBrowser.tsx'
import { zh } from '../src/client/workspace/locales.ts'
...
describe('product workspace settings entry', () => {
  it('opens settings for the actual clicked Workspace and retains its menu actions', async () => {
    const opened: string[] = []
    const browser = mount({ useWorkspaces: hook(workspaceState([workspace('one', []), workspace('two', [])])),
      renderSlot: ((name: string, owner: { workspaceId?: string; open?: boolean }) => {
        if (name === 'sidebar.workspaces.imSettings' && owner.workspaceId !== undefined) { opened.push(owner.workspaceId); return <div data-testid="im-workspace-settings">{owner.workspaceId}</div> }
        return null
      }) as never,
    })
    fireEvent.click(screen.getByRole('button', { name: t('actions.workspace.aria', { name: 'two' }) }))
    fireEvent.click(await screen.findByRole('menuitem', { name: zh['settings.title'] }))
    expect(await screen.findByTestId('im-workspace-settings')).toHaveProperty('textContent', 'two')
    expect(opened).toContain('two')
    expect(browser.props.renameWorkspace).not.toHaveBeenCalled()
    expect(browser.props.deleteWorkspace).not.toHaveBeenCalled()
  })
})
