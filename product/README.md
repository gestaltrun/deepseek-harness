...
The repository snapshot corpus includes an authored Web replay for the same runtime-owned admission path. The synthetic transport contributes one page through `receivePage`; the product runtime creates the only root Session, and the shipped `standard` preset replays three scripted model responses through history, send, and final-answer steps. The adapter compares the complete Session, system prompt, and tool schemas, and separately checks the sent outbox item. This keyless scenario uses no real account or model.
