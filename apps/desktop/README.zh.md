...
Electron 根据应用 locale 选择类型化的英文或中文桌面壳文案，并以英文作为 fallback。菜单、原生对话框、启动页与插件管理渲染进程使用同一 locale 数据；仓库的 Client UI i18n gate 会检查这些桌面源文件。应用菜单包含本地化的编辑操作，因此在自定义应用菜单替换 Electron 默认菜单后，剪切、复制、粘贴和全选仍保留原生快捷键。
