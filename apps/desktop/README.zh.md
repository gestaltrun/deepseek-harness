...
桌面应用是包裹 dsh Web UI 的 Electron 壳。内置的上游 Node.js 子进程启动已安装的 dsh 项目，带版本的分帧字节管道在没有外层 Base64 信封的情况下承载 Fetch 请求与流式响应，Node IPC 承载生命周期控制，`dsh-app://` 则提供与后端版本匹配的客户端资源。
...
| 通信 | 监听 Web 服务会引入端口归属、认证、CORS 与暴露风险；Electron 与上游 Node.js 之间也需要明确的跨进程协议。 | 原生界面不使用 Web 端口。`dsh-app://` 承载 Web 资源和 Fetch 流量；分帧字节管道以背压传输有界请求与响应分块，Node IPC 只承载子进程生命周期控制。 |
...
Desktop 默认以无头模式启动 Ego 的独立浏览器（`egoCliArgs: '--headless'`）。如需在下一次冷启动时显示浏览器窗口，在插件设置中将 Ego 的 `egoCliArgs` 显式设为空字符串。已有启动参数与配置表达式会保留；共享 Web profile 使用自身默认值。
...
手动触发的 [Desktop Release 工作流](../../.github/workflows/desktop-release.yml)接受准确的提交、dsh 与 Desktop 共享的版本、`test` 或 `production`，以及三种操作之一。`validate` 执行无凭据的请求检查。`candidate` 接受已包含在 `master` 中的提交，并创建经过签名和公证的 macOS 产物以及未签名 Windows 安装包，但不会将其加入更新频道。`publish` 使用相同的源码限制，上传 macOS 和 Windows 安装包，并为生产部署创建 GitHub Release。`candidate` 和 `publish` 始终在 GitHub 托管的 `windows-latest` 上打包未签名 Windows 安装包。发布会把该安装包上传到所选 OSS feed；生产发布还会把它附加到 GitHub Release。签名操作要求 macOS arm64 和 x64；签名 Windows x64 仍可明确选择，但缺少已配置的硬件签名输入时会失败。
...
该命令要求设置 `DSH_DESKTOP_APP_ID`、所选公开 feed URL，并具备常规构建依赖，包括编译原生模块所需的 Python 和 Visual C++ 构建工具。Python 不在 `PATH` 中时，将 `PYTHON` 设置为其可执行文件路径。命令将安装包写入 `.desktop-build/targets/win-x64/unsigned-artifacts/`，清除签名凭据，并写入更新元数据和完成记录。它不需要 EV 凭据。Desktop Release 的 `candidate` 和 `publish` 操作会在 GitHub 托管的 `windows-latest` 上运行相同命令，并将安装包保留为 Actions 产物。`publish` 会把该安装包上传到所选 OSS feed；生产发布还会把它附加到 GitHub Release。签名打包命令仍遵循硬件签名要求。
