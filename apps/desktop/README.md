...
Electron chooses typed English or Chinese shell copy from its application locale and falls back to English. Menus, native dialogs, the startup page, and the plugin-management renderer use the same locale payload; the repository Client UI i18n gate checks these desktop sources. The application menu includes locale-owned Edit actions so Cut, Copy, Paste, and Select All keep their native accelerators after the custom Application menu replaces Electron's default.
