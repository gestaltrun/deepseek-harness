...
| Transport | A listening Web service adds port ownership, authentication, CORS, and exposure concerns; Electron and upstream Node.js also need an explicit cross-process protocol. | The native UI uses no Web port. `dsh-app://` carries Web assets and Fetch traffic; framed byte pipes carry bounded request and response chunks with backpressure, while Node IPC carries only child lifecycle control. |
...
Desktop starts Ego’s separate browser in headless mode by default (`egoCliArgs: '--headless'`). To request a browser window on the next cold start, set Ego’s `egoCliArgs` to an explicit empty string in plugin settings. Explicit user arguments and configuration expressions are preserved; the shared Web profile keeps its own defaults.
...
The manually dispatched [Desktop Release workflow](../../.github/workflows/desktop-release.yml) accepts an exact commit, the version shared by dsh and Desktop, `test` or `production`, and one of three operations. `validate` performs credential-free request checks. `candidate` accepts a commit contained in `master` and creates signed and notarized macOS artifacts plus an unsigned Windows installer without adding them to an update channel. `publish` applies the same source restriction, uploads macOS and Windows installers, and creates a GitHub Release for a production deployment. Candidate and publish always package the unsigned Windows installer on GitHub-hosted `windows-latest`. Publication uploads that installer to the selected OSS feed; production publication also attaches it to the GitHub Release. macOS arm64 and x64 remain required signed targets; signed Windows x64 remains explicitly selectable and fails unless the configured hardware-backed signing inputs are available.
...
The command requires `DSH_DESKTOP_APP_ID`, the selected public feed URL, and the normal build dependencies, including Python and Visual C++ build tools for native modules. Set `PYTHON` to the Python executable when it is absent from `PATH`. It writes the installer to `.desktop-build/targets/win-x64/unsigned-artifacts/`, strips signing credentials, and writes updater metadata plus a completion record. It does not require EV credentials. The Desktop Release `candidate` and `publish` operations run this same command on GitHub-hosted `windows-latest` and keep the installer as an Actions artifact. `publish` uploads that installer to the selected OSS feed; production publication also attaches it to the GitHub Release. The signed packaging command retains its hardware-backed signing requirements.
