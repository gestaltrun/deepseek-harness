...
Upload additionally requires `DESKTOP_RELEASE_OSS_BUCKET`, `DESKTOP_RELEASE_OSS_ENDPOINT`, and `DESKTOP_RELEASE_ALIYUN_REGION`. The repository-scoped Alibaba Cloud OIDC action supplies `ALIBABA_CLOUD_ACCESS_KEY_ID`, `ALIBABA_CLOUD_ACCESS_KEY_SECRET`, and `ALIBABA_CLOUD_SECURITY_TOKEN` only to the publish job. Packaging removes those temporary credentials from every subprocess and needs only the selected feed URL.
