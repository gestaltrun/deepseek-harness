...
- macOS publication requires signing and notarization, and Windows publication requires the existing hardware-backed signing inputs.
- Packaging does not receive OSS credentials, and publication does not accept a commit outside `master`.
- Every target's immutable payloads exist before any channel metadata is replaced.
