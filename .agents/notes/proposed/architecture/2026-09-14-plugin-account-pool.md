...
An exclusive root lock rejects concurrent owners. `stateRoot/auth/` holds engine-owned account files, including GLM Coding Plan. The superseded product GLM ledger, generation TLS, and credential export are replaced by [engine-owned GLM over auth-files](2026-09-15-account-pool-engine-owned-glm.md). Private directories use mode 0700 and files use 0600 where supported. Cleanup removes only the terminated generation's random directory below `stateRoot/generations/`; stable credentials and unrelated data remain. No default user CLIProxyAPI home is read.
