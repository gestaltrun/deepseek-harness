...
| Typert build inputs and runtime Loader | The pinned compiler requires a workspace layout and protocol declaration project; product-owned staging already supplies both. Packed runtime calls remain unverified. | Keep the upstream compiler and Loader unchanged. Generate with public declarations in private build inputs, then verify the packed package and real Remote calls. Only a reproduced runtime gap may justify an upstream exception. |
...
### Product-owned Remote generation

The pinned public `@deepseek-ai/dsh-typert-generator@0.1.5-rc.2` discovers projects under the compiler root's `packages` directory and recognizes Remote markers through a registered workspace protocol declaration project. Direct product-project selection or source relocation without that declaration project can produce no Remote artifacts. These are build-time discovery and declaration-identity constraints, not evidence that the runtime Loader cannot load an external plugin.

The product build helper copies unchanged API and referenced product source plus the real published protocol declarations into a private temporary compiler tree. It selects only the API package, emits Host reflection and Remote Client artifacts, and removes the private tree afterward. Generated files retain formal package imports and relative declaration maps. Checks reject empty or missing Remote output, require the complete expected method set, reject private absolute paths, and compare output bytes across two independent roots; invalid export and concurrent-build cases are covered. The verified initial real-API checkpoint contains 13 Remote methods; adding a method must update the expected set and rerun its generation checks. This checkpoint establishes generation, not packed invocation or GUI acceptance.

DTOs must be declared and exported by their owning package through a public Client-safe non-root entry such as `./types`, with real JavaScript and declaration outputs. Moving a type-only re-export into the Client does not establish its public owner for the generator. The product workspace override pins Zod to `4.4.3`, matching the baseline schema interfaces while leaving upstream package implementations unchanged. The helper, protocol declaration copies, and compiler remain build inputs rather than runtime package payloads.
...
The confirmed placement is `product/`, with upstream packages preserved by default. The accepted implementation direction covers runtime/Provider/API/UI roles, product-owned workspace settings adaptation, aggregate CAS and partial-result semantics, and Desktop/Web product composition. Product-only entry fidelity, packed Host/Client Remote invocation, legacy data import scope, live provider receipts, and derived-work attribution remain evidence obligations. The verified generation checkpoint does not settle those runtime obligations. Any upstream exception requires the user to see the no-change impact and product-only alternatives before approving that exact modification; this proposal authorizes no such exception.
