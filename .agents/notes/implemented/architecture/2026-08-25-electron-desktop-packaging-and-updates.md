...
`DSH_DESKTOP_AUTO_UPDATE_ENV` selects the test deployment by default or the production deployment for the target-specific generic-provider URL and OSS prefix. Each deployment supplies its public feed root and object prefix through release-environment variables, while the shared bucket, endpoint, and region remain deployment configuration. Packaging resolves only the public updater URL, disables electron-builder publishing, removes the short-lived Alibaba Cloud credential fields from every subprocess, and writes a completion record only after electron-builder and every signing or notarization hook succeeds. The manual Desktop Release workflow accepts an exact commit and coupled Desktop/dsh version; candidate packaging keeps the official signing requirements, while publication accepts only a commit contained in `master` and obtains OSS authority through a repository-scoped OIDC role. Target upload requires the completion record, root dsh version, Desktop version, version-derived channel metadata, artifact names, sizes, and SHA-512 values to agree before sending data. It uploads and verifies every selected immutable updater payload and separate blockmap before replacing any selected channel metadata. Immutable keys refuse different bytes and retain their stored SHA-512 for retry verification; channel metadata remains the only replaceable object. A production publication creates a `gestalt-v<version>` GitHub Release with OSS installer links and refuses to reuse a tag or draft owned by another commit. Stable versions use the `latest` metadata name; prereleases use the first semantic-version prerelease identifier. NSIS embeds its blockmap in the signed executable; the macOS ZIP carries a separate blockmap. Both let electron-updater download changed blocks when supported, while application replacement and the local pnpm package operation remain separate operations.
...
- The native carrier opens no listener; remote access has a separate authenticated listener, and the sandboxed renderer cannot access arbitrary filesystem or Electron APIs.
