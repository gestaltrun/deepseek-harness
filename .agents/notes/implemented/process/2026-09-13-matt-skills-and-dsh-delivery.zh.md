...
通用工程技能可能选择与使用它的仓库不同的事项跟踪器、测试策略、Agent 生命周期或文档归属。上游刷新也可能移除本地验收要求。跨仓库插件交付需要超出单仓库 CI 范围的源码和制品证据。只对比上游目录会漏掉承载用户选择的仓库本地参考文件。
...
**复制旧 fork 的整个工作流。** 其跟踪器、启动脚本、无条件供应商选择和 CI 控制不描述当前检出版本。可移植的上下文和插件参考文档改为解析当前归属。
