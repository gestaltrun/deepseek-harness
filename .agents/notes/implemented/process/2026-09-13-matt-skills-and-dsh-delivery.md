...
Generic engineering skills can select a different tracker, test policy, agent lifecycle, or document owner from the repository using them. An upstream refresh can also remove local acceptance requirements. Plugin delivery across repositories requires source and artifact evidence beyond one repository’s CI. Comparing only the upstream catalog misses repository-local references that preserve user choices.
...
[The migration audit](../../../skills/MIGRATION.md) accounts for all 114 files in the 41 selected source skill trees. ODD retains [session roles](../../../skills/orchestrate-dsh-delivery/references/session-roles.md), [provider-neutral delegation](../../../skills/orchestrate-dsh-delivery/references/delegation-routing.md), and the original [CLIProxyAPI model priorities](../../../skills/orchestrate-dsh-delivery/references/model-routing-cliproxyapi.md). The priority table applies only to that permitted provider after live capability checks; Root never selects a new model for itself. Desktop retains [CUA evidence levels](../../../skills/dsh-desktop-test-instance/CUA-AVAILABILITY.md), and both workflow metadata files are tracked. Source hashes distinguish unchanged policy from current-host adaptations.

Normal delivery requests authorize repository delivery through evidence-backed merge, subject to explicit user limits and separate message/release authorization. Root delegates implementation and tests even for a small delivery. The [delivery ledger](../../../skills/orchestrate-dsh-delivery/references/delivery-ledger.md) binds accepted scope, owners, exact remote inputs and results, a draft PR, evidence, states, blockers, and next checks before dispatch and after integration. Explicit local-only work retains local records.

Supervision checks known prerequisites before expensive runs and pauses unchanged retries after a repeated failure for an owner-local discriminating experiment. Kept retrospective items retain their repository-local lesson id, first-occurrence record, and repeat count; this local archive does not authorize global-memory writes.
...
**Copy the old fork’s entire workflow.** Its tracker, launch scripts, unconditional provider selection, and CI controls do not describe this checkout. Portable context and plugin references resolve the current owners.
