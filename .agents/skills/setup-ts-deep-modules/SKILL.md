...
For the vocabulary (deep module, interface, seam, depth), use [codebase-design](../codebase-design/SKILL.md) and use its language throughout.
...
Update the existing package-layout or dependency-rule owner with the rules actually adopted and the verification command actually run. Preserve the repository's grouped packages, public exports, compiler programs, test layout, and barrel conventions; the example layout is not permission to replace them.
...
**Done when:** the owning documentation states the verified local rule and the real agent-instruction owner links to it.
