...
Use the fewest sessions that preserve ownership and independent judgment. Reuse each owner through coherent fixes; do not create a session for every ticket, error, or review comment. [ODD](../SKILL.md) owns the demand and acceptance order; [delegation routing](delegation-routing.md) owns dispatch, model capability, continuation, and supervision. A small local change may use one implementation owner under ODD's lightweight path; coordinated delivery uses the responsibilities below.

| Role | Responsibility |
| --- | --- |
| Root | Keeps the demand, accepted scheme/design, dependency graph, decisions, evidence, and user communication. Root model selection belongs to the user; the coordinator never routes or changes itself. For coordinated delivery it assigns implementation and integration to their owners, rather than becoming another writer. It reviews ticket GIFs and supporting evidence for functional completeness and synthesizes retrospective choices. |
