...
An ODD request to implement, fix, continue, or land repository work authorizes the normal delivery actions within that accepted scope: isolated branches/worktrees, edits, commits, pushes, a draft delivery PR, review fixes, and merge after the required evidence and retrospective decisions pass. Explicit limits such as local-only work, no push, or stopping before merge override that default. Preserve those limits and existing authorization without repeated questions; external messages still require the applicable explicit authorization. Tags, stable package publication, deployment, signing, and formal releases retain their per-release authorization. Related Gestaltrun repositories follow the demand's coordinated release set.

Complete intake when the accepted outcome, non-goals, source identities, unresolved product choices, and authorized endpoint are explicit. Small local changes can use one implementation owner and focused checks without manufacturing a specification or ticket graph; Root remains the coordinator rather than becoming that writer.
...
Before dispatching implementation, use [the delivery ledger and handoff procedure](references/delivery-ledger.md). It owns the clean planning workspace, published accepted inputs, exact remote revision, draft PR, fixed delivery-unit records, and ready frontier. When push or remote publication is outside the authorized scope, retain a frozen local integration revision and accepted local ledger instead.
...
Choose only the roles the work needs: scheme, UI, module implementation, integration/environment, Quality, and independent review. Root delegates implementation, document/environment edits, product tests, and integration to their owners, including late review, CI, acceptance, and retrospective fixes. Root may investigate, write briefs, maintain the ledger, query status/provenance, and perform authorized PR coordination; it does not take over a missing writer. If a supported writer executor or isolated writable workspace cannot be provided, stop that implementation path and report the capability gap. The primary author is not the independent reviewer. Apply permitted model priorities only after actual capability checks; report unmet independence rather than claiming it.

For one coordinated feature, use one integration branch. When remote publication is authorized, one draft PR closes its specification and delivered tickets; otherwise the frozen local revision and accepted local ledger record delivery. Module writers do not create a PR per ticket. Independent delivery scopes remain separate. Use a PR stack only when actual PR dependencies require it, following [dsh-merging-stacked-prs](../dsh-merging-stacked-prs/SKILL.md). Across repositories, use exact source/package dependencies and the delivery graph instead of a same-repository stack.
