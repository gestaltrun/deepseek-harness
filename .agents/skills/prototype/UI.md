...
1. For a native Desktop prototype, reuse the existing authorized native computer-use owner against an isolated Electron through the current supported launch path. An upstream comparison may use the explicitly requested baseline/candidate pair; creating a user-owned Codex task still requires an explicit request. A Web-only prototype may use an isolated headless browser. Mock data for the new function remains acceptable.
